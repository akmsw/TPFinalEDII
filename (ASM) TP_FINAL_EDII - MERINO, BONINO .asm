...
;-------------------RUTINA INTERRUPCIÓN-----------------------------------------
	  ...
	  RUT_IN    MOVWF   W_TEMP          ;Salvamos contexto.
		    SWAPF   STATUS,W
		    MOVWF   STATUS_TEMP
		    BTFSC   INTCON,RBIF	    ;Si la interrupción fue por RB<7:1>, vamos a la rutina
		    ...
		    GOTO    TMR0_IN
		    BTFSC   PIR1,RCIF	    ;Si fue por recepción por puerto serie, vamos a la de RX.
		    GOTO    RECEP_IN
...
;-------------------FIN RUTINA INTERRUPCIÓN-------------------------------------
     ...
     DEV_CONTEXT    SWAPF   STATUS_TEMP,W   ;Devolución de contexto.
		    MOVWF   STATUS
		    SWAPF   W_TEMP,F
		    SWAPF   W_TEMP,W
		    RETURN
